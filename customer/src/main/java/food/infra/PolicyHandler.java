package food.infra;

import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import food.config.kafka.KafkaProcessor;
import food.domain.*;
import javax.naming.NameParser;
import javax.naming.NameParser;
import javax.transaction.Transactional;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.cloud.stream.annotation.StreamListener;
import org.springframework.messaging.handler.annotation.Payload;
import org.springframework.stereotype.Service;

@Service
@Transactional
public class PolicyHandler {

    @Autowired
    OrderRepository orderRepository;

    @StreamListener(KafkaProcessor.INPUT)
    public void whatever(@Payload String eventString) {}

    @StreamListener(
        value = KafkaProcessor.INPUT,
        condition = "headers['type']=='StartCook'"
    )
    public void wheneverStartCook_State(@Payload StartCook startCook) {
        StartCook event = startCook;
        System.out.println("\n\n##### listener State : " + startCook + "\n\n");

        // Sample Logic //
        Order.state(event);
    }

    @StreamListener(
        value = KafkaProcessor.INPUT,
        condition = "headers['type']=='PickDelivery'"
    )
    public void wheneverPickDelivery_State(@Payload PickDelivery pickDelivery) {
        PickDelivery event = pickDelivery;
        System.out.println(
            "\n\n##### listener State : " + pickDelivery + "\n\n"
        );

        // Sample Logic //
        Order.state(event);
    }
}
